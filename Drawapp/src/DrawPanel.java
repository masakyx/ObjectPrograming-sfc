...
public class DrawPanel<Oval2D> extends Panel {
	...
	public void setMode( String m ) {
		...
		else if (m.equals("Oval")) {current =Mode.Oval;}
	...
	public void paint(Graphics g) {
		...
		if (currentOval !=null) {g2.draw(currentOval);}
	...
	class MyMouseAdapter extends MouseAdapter {
		public void mousePressed( MouseEvent me ) {
			...
			else if (getMode()== Mode.Oval) {createOval(me.getX(),me.getY());}
		...
		public void mouseReleased( MouseEvent me ) {
			...
			if ( getMode() == Mode.Rect ) { registerRect(me.getX(),me.getY());}
			if (getMode()==Mode.Oval){registerOval(me.getX(),me.getY());}
	...
	class MyMouseMotionAdapter extends MouseMotionAdapter {
		public void mouseDragged( MouseEvent me ) {
			...
			else if (current == Mode.Oval){redrawOval(me.getX(),me.getY());}
	...
	// 現在描画している円
		Ellipse2D  currentOval;
		// ovalを作画するときのメソッド
		void createOval( int mx, int my ) {
			currentOval = new Ellipse2D.Double( mx, my, 0, 0 );
			startp = new Point2D.Double( mx, my );
		}
		void redrawOval( int mx, int my ) {
			...
			double w = Math.abs( sx - mx );
			double h = Math.abs( sy - my );
			currentOval = new Ellipse2D.Double( x, y, w, h );
		}
		void registerOval( int mx, int my ) {
			...
			double w = Math.abs( sx - mx );
			double h = Math.abs( sy - my );
			objects.add( new DrawOval( x, y, w, h, colorlist[ framecolor.getSelectedIndex() ], colorlist[ fillcolor.getSelectedIndex() ] ) );
			currentOval = null;
			startp = null;
		}
